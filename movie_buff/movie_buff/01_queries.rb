...
def harrison_ford
  ...
  Actor.select("movies.id", :title).joins(:movies, :castings)
  .where(name: 'Harrison Ford').where("castings.ord > 1").distinct
end
...
def biggest_cast
  ...
  .group(:id).order("COUNT(actor_id) DESC").limit(3)
end
...
def directed_by_one_of(them)
  ...
  # Movie.select(:id, :title).joins("JOIN actors ON actors.id = movies.director_id")
